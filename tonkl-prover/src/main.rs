...
fn cmd_compute() {
    ...
    let result: serde_json::Value = match op {
        "commitment" => {
            ...
            serde_json::json!({
                "commitment": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&cm)))
            })
        }
        "nullifier" => {
            ...
            serde_json::json!({
                "nullifier": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&nf)))
            })
        }
        "derive_pk" => {
            ...
            serde_json::json!({
                "pk_x": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&pk_x))),
                "pk_y": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&pk_y)))
            })
        }
        "full_note" => {
        ...
        "hash_2" => {
            ...
            serde_json::json!({
                "hash": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&h)))
            })
        }
        "merkle_tree" => {
                ...
                .map(|p| {
                    serde_json::json!({
                        ...
                        "siblings": p.siblings.iter().map(|s| {
                            format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(s)))
                        }).collect::<Vec<String>>()
                    })
                })
                .collect();
            ...
            serde_json::json!({
                "root": format!("0x{}", hex::encode(tonkl_prover::fe_to_be_32(&result.root))),
                "paths": paths_json,
