...
Run from the tonkl-prover directory:
    python3 scripts/generate_reference_fixture.py

Requires:
  - nargo on PATH
  - tonkl-hasher compiled (nargo compile in ../tonkl-hasher)
  - tonkl-transfer compiled (nargo compile in ../tonkl-transfer)
...
TRANSFER_DIR  = os.path.join(os.path.dirname(PROVER_DIR), "tonkl-transfer")
...
def main():
    ...
    if not os.path.isfile(CIRCUIT_JSON):
        ...
        print("    Run: cd ../tonkl-transfer && nargo compile")
        sys.exit(1)
