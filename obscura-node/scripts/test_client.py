...
responses. Uses real proof artifacts from obscura-mint.
...
def setup_vk_dir() -> Path:
    ...
    d = Path(tempfile.mkdtemp(prefix="obscura-test-vks-"))
...
def start_node(vk_path: Path) -> subprocess.Popen:
    ...
    global data_dir
    data_dir = tempfile.mkdtemp(prefix="obscura-test-data-")
...
def main():
    ...
    if not NODE_BIN.exists():
        ...
        print("  Run: cd obscura-node && cargo build --release")
        sys.exit(1)
