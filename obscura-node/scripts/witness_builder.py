...
Usage:
    ...
    builder.write_toml(toml, "obscura-transfer/Prover.toml")
"""
...
# ─────────────────────────────────────────────────────────────────────
# Crypto Helper (wraps obscura-prover compute)
# ─────────────────────────────────────────────────────────────────────

class CryptoHelper:
    """
    Computes Poseidon2 hashes, commitments, nullifiers, and key derivation
    by calling the `obscura-prover compute` subcommand as a subprocess.

    Requires the obscura-prover binary to be built and accessible.
    """
    ...
    def __init__(self, prover_bin: Optional[str] = None):
        if prover_bin is None:
            ...
            candidate = script_dir.parent.parent / "obscura-prover" / "target" / "release" / "obscura-prover"
            if candidate.exists():
                self.prover_bin = str(candidate)
            else:
                self.prover_bin = "obscura-prover"  # Assume on PATH
        else:
            self.prover_bin = prover_bin

    def _compute(self, request: dict) -> dict:
        """Call obscura-prover compute with a JSON request via stdin."""
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"obscura-prover compute failed: {result.stderr.strip()}"
            )
