...
The full spend cycle running against a live node:
  ...
  4. Compute note data for two minted notes using obscura-prover compute
  5. Build transfer witness with live merkle proofs from the node
  6. Generate transfer proof via obscura-prover prove
...
Prerequisites:
  ...
  - obscura-prover built: cargo build --release
  - Mint proof artifacts: obscura-mint/target/proof/{proof, public_inputs}
  - Transfer circuit JSON: obscura-transfer/target/obscura_transfer.json
...
# ─────────────────────────────────────────────────────────────────────
# The mint used these recipient secret keys (see obscura-tree/generate_mint_witness.py):
...
def setup_vk_dir() -> Path:
    d = Path(tempfile.mkdtemp(prefix="obscura-e2e-vks-"))
...
def start_node(vk_path: Path) -> subprocess.Popen:
    global data_dir
    data_dir = tempfile.mkdtemp(prefix="obscura-e2e-data-")
...
def main():
    ...
    # ── [3] Compute note data ─────────────────────────────────────────
    print()
    print("[3/8] Computing note data via obscura-prover compute...")
    ...
    # Build JSON witness (obscura-prover expects JSON on stdin)
    ...
    proof_work_dir = Path(tempfile.mkdtemp(prefix="obscura-e2e-transfer-"))
    ...
    # Pipe JSON witness to obscura-prover prove
