...
Prerequisites:
  - obscura-node built:    cd obscura-node && cargo build --release
  - obscura-prover built:  cd obscura-prover && cargo build --release
...
def setup_vk_dir() -> Path:
    ...
    d = Path(tempfile.mkdtemp(prefix="obscura-comp-vks-"))
...
def start_node(vk_path: Path) -> subprocess.Popen:
    global data_dir
    data_dir = tempfile.mkdtemp(prefix="obscura-comp-data-")
...
def main():
    ...
    split_proof_dir = Path(tempfile.mkdtemp(prefix="obscura-comp-split-"))
    try:
    ...
    # Build transfer witness (JSON for obscura-prover)
    print()
    ...
    # Prove via obscura-prover (stdin JSON)
    xfer_proof_dir = Path(tempfile.mkdtemp(prefix="obscura-comp-xfer-"))
    ...
    merge_proof_dir = Path(tempfile.mkdtemp(prefix="obscura-comp-merge-"))
    try:
