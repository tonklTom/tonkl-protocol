...
MINT_PROOF = ROOT / "obscura-mint" / "target" / "proof" / "proof"
MINT_PI = ROOT / "obscura-mint" / "target" / "proof" / "public_inputs"

sys.path.insert(0, str(SCRIPT_DIR))
from node_client import TonklClient, ConnectionError as NodeConnectionError
from obscura_wallet import (
    NodeWallet, find_vk,
...
def setup_vk_dir():
    d = Path(tempfile.mkdtemp(prefix="obscura-wtest-vks-"))
...
def start_node(vk_path):
    global data_dir
    data_dir = tempfile.mkdtemp(prefix="obscura-wtest-data-")
...
def main():
    ...
    db_path = Path(tempfile.mktemp(suffix=".db", prefix="obscura-wtest-"))
    wallet = NodeWallet(
    ...
    if has_nacl:
        from obscura_wallet import _derive_scan_keypair, encrypt_note_data, decrypt_note_data
        ...
        db_path_recipient = Path(tempfile.mktemp(suffix=".db", prefix="obscura-recipient-"))
        ...
        from obscura_wallet import BackgroundScanner

        # Create a fresh recipient wallet with background scanning
        db_path_bg = Path(tempfile.mktemp(suffix=".db", prefix="obscura-bgscan-"))
    ...
    db_path_seed = Path(tempfile.mktemp(suffix=".db", prefix="obscura-seed-"))
    ...
    db_path_restored = Path(tempfile.mktemp(suffix=".db", prefix="obscura-restored-"))
    ...
    from obscura_wallet import HAS_SQLCIPHER, _derive_db_key
    ...
    if HAS_SQLCIPHER:
        ...
        db_path_enc = Path(tempfile.mktemp(suffix=".db", prefix="obscura-enc-"))
        ...
        db_path_fb = Path(tempfile.mktemp(suffix=".db", prefix="obscura-fb-"))
    ...
    db_path_ma = Path(tempfile.mktemp(suffix=".db", prefix="obscura-multi-asset-"))
    ...
    db_path_err = Path(tempfile.mktemp(suffix=".db", prefix="obscura-err-"))
    ...
    db_path_faucet = Path(tempfile.mktemp(suffix=".db", prefix="obscura-faucet-"))
    faucet_sk = "0xface70"
    ...
    wallet_src_path = SCRIPT_DIR / "obscura_wallet.py"
    ...
    if readme_path.exists():
        ...
        check("README has wallet command example", "obscura_wallet" in readme)
    ...
    wallet_src = (SCRIPT_DIR / "obscura_wallet.py").read_text()
