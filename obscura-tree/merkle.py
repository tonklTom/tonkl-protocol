...
Sparse depth-32 binary Merkle tree backed by the obscura-prover Rust binary
...
class MerkleTree:
    ...
    def __init__(self, prover_path=None):
        """
        Args:
            prover_path: Path to the compiled obscura-prover binary.
                         Defaults to ../obscura-prover/target/release/obscura-prover
                         relative to this file.
        """
        if prover_path is None:
            here = os.path.dirname(os.path.abspath(__file__))
            prover_path = os.path.join(
                here, "..", "obscura-prover", "target", "release", "obscura-prover"
            )
    ...
    def _check_prover(self):
        ...
        if not os.path.isfile(self.prover_path):
            raise FileNotFoundError(
                f"obscura-prover not found at {self.prover_path}\n"
                "  Build it: cd obscura-prover && cargo build --release"
            )
    ...
    def _compute(self):
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"obscura-prover merkle_tree failed (exit {result.returncode}):\n"
...
def compute_note(prover_path, sk, value, asset_id, rho):
    ...
    Args:
        prover_path: Path to obscura-prover binary.
