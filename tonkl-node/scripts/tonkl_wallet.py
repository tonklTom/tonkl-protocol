...
Supports all four circuit types:
  send   (transfer 2-in/2-out)  via tonkl-prover
...
HD_DERIVATION_DOMAIN = b"Tonkl::note_sk_v1"
...
def _derive_scan_keypair(spending_sk: str) -> Tuple[bytes, bytes]:
    ...
    The scan private key = SHA256(spending_sk || "tonkl-scan-v1")[:32],
    ...
    seed = hashlib.sha256(
        (spending_sk + ":tonkl-scan-v1").encode()
    ).digest()
    sk = PrivateKey(seed)
...
def _open_encrypted_db(db_path: str, passphrase: str):
    ...
    try:
        ...
        if not os.path.exists(salt_path):
            legacy_salt = b"tonkl-wallet-v1-sqlcipher"
...
class NodeWallet:
    ...
    All cryptography is delegated to the tonkl-prover binary.
    """
    ...
    def mint_token(
        ...
        # bb prove
        mint_json = mint_dir / "target" / "tonkl_mint.json"
        mint_vk = find_vk("tonkl-mint")

        import tempfile
        tmp_dir = Path(tempfile.mkdtemp(prefix="tonkl-mint-"))
    ...
    def send(
        ...
        # Prove via tonkl-prover (with retry)
        print("  Generating transfer proof...")
        proof_dir = Path(tempfile.mkdtemp(prefix="tonkl-wallet-xfer-"))
    ...
    def split(
        ...
        try:
            ...
            proof_dir = Path(tempfile.mkdtemp(prefix="tonkl-wallet-split-"))
    ...
    def merge(
        ...
        try:
            ...
            proof_dir = Path(tempfile.mkdtemp(prefix="tonkl-wallet-merge-"))
