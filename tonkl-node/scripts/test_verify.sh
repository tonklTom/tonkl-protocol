...
VK_DIR="$ROOT/tonkl-node/test-vks"
DATA_DIR="/tmp/tonkl-test-$$"
PORT=9199
PASS=0
FAIL=0
...
for circuit in tonkl-transfer tonkl-merge tonkl-split tonkl-mint; do
    ...
    if [[ -n "$VK_FILE" ]]; then
        ...
        CIRCUIT_JSON="$CIRCUIT_DIR/target/${circuit##tonkl-}.json"
...
cp "$(find_vk tonkl-transfer)" "$VK_DIR/transfer/vk"
cp "$(find_vk tonkl-merge)" "$VK_DIR/merge/vk"
cp "$(find_vk tonkl-split)" "$VK_DIR/split/vk"
cp "$(find_vk tonkl-mint)" "$VK_DIR/mint/vk"
...
PROOF_HEX="0x$(xxd -p -c0 "$ROOT/tonkl-mint/target/proof/proof")"
PI_HEX="0x$(xxd -p -c0 "$ROOT/tonkl-mint/target/proof/public_inputs")"
...
PI_COUNT=$(($(wc -c < "$ROOT/tonkl-mint/target/proof/public_inputs") / 32))
...
for i in $(seq 0 $((PI_COUNT - 1))); do
    OFFSET=$((i * 32))
    CHUNK="0x$(xxd -p -l 32 -s $OFFSET "$ROOT/tonkl-mint/target/proof/public_inputs" | tr -d '\n')"
