...
MINT_CIRCUIT_JSON = MINT_DIR / "target" / "tonkl_mint.json"

# ⚠️ TESTNET ONLY — rotate before any public deployment
...
def build_genesis(
    ...
    genesis_notes = []
    block_num = 0
    tmp_dir = Path(tempfile.mkdtemp(prefix="tonkl-genesis-"))
    ...
    if output_path is None:
        output_path = str(ROOT / "tonkl-node" / "genesis.json")
...
def main():
    ...
    parser.add_argument(
        "--output", "-o",
        default=None,
        help="Output genesis.json path (default: tonkl-node/genesis.json)",
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
