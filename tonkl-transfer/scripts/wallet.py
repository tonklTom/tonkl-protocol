#!/usr/bin/env python3
"""
Tonkl Wallet -- SQLite-backed note tracker, balance engine, and transaction builder.

Integrates with:
  - tonkl-prover compute  (Poseidon2 hashing, pk derivation)
...
Usage:
    ...
    w = Wallet()                          # opens ~/.tonkl/wallet.db
...
_PROVER_SEARCH_PATHS = [
    Path(__file__).resolve().parent.parent.parent / "tonkl-prover" / "target" / "release" / "tonkl-prover",
    Path.home() / ".tonkl" / "bin" / "tonkl-prover",
]

def _find_prover() -> Path:
    """Find the tonkl-prover binary."""
    for p in _PROVER_SEARCH_PATHS:
        if p.exists() and os.access(p, os.X_OK):
            return p
    raise FileNotFoundError(
        "tonkl-prover binary not found. Build with: cd tonkl-prover && cargo build --release"
    )
...
def _compute(op_json: dict) -> dict:
    """
    Call `tonkl-prover compute` with JSON stdin, return parsed JSON output.
    ...
    for attempt in range(1, COMPUTE_MAX_RETRIES + 1):
        try:
            ...
            last_err = f"tonkl-prover compute timed out ({COMPUTE_TIMEOUT_SECS}s)"
        ...
        if proc.returncode != 0:
            ...
            last_err = f"tonkl-prover compute failed (exit {proc.returncode}): {stderr}"
        ...
        try:
            ...
            raise ProverError(
                f"tonkl-prover compute returned invalid JSON: {e}\n"
...
class Wallet:
    ...
    def __init__(self, db_path: Optional[str] = None, passphrase: Optional[str] = None):
        ...
        Args:
            db_path:    Path to wallet.db (default: ~/.tonkl/wallet.db)
    ...
    def _build_prover_dict(
        ...
        # But that requires nargo + tonkl-hasher. Instead, let's add a
        # poseidon2_hash op to the compute subcommand.
        #
        # SIMPLER APPROACH: call tonkl-prover compute with the full note
        ...
        print("  [hasher] Computing hashes via tonkl-hasher...")
    ...
    def execute_send(
        ...
        Steps:
          ...
          2. Compute all hashes via tonkl-hasher
          3. Call tonkl-prover prove (with retries on transient failure)
        ...
        try:
            ...
            raise ProverError(
                ...
                f"Check that nargo is installed and tonkl-hasher compiles."
            ) from e
        ...
        # Step 3: Call tonkl-prover prove (with retries)
        gw_path = os.path.join(os.path.dirname(__file__), "generate_witness.py")
        spec = importlib.util.spec_from_file_location("generate_witness", gw_path)
        gw = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(gw)
        ...
        circuit_json = os.path.join(
            gw.TRANSFER_DIR, "target", "tonkl_transfer.json"
        )
        ...
        if not os.path.isfile(circuit_json):
            ...
            raise FileNotFoundError(
                ...
                f"Run: cd tonkl-transfer && nargo compile"
            )

        last_prove_err = None
        for attempt in range(1, max_prove_retries + 1):
            print(f"  [prove] Calling tonkl-prover prove"
    ...
    def import_receipt(self, receipt: dict, sk_hex: str) -> Note:
        ...
        """
        if receipt.get("type") != "tonkl_note_receipt":
...
def _cli():
    ...
    parser = argparse.ArgumentParser(description="Tonkl Wallet")
    parser.add_argument(
