...
Diagnostic harness: run the Rust obscura-prover and `nargo execute` on
...
Output:
  ...
  target/witness_rust.gz    -- produced by ./obscura-prover
...
CIRCUIT_JSON = os.path.join(TARGET_DIR, "obscura_transfer.json")
...
def run_rust_on_prover_dict(prover_dict: dict, out_witness_gz: str) -> None:
    """Pipe the SAME dict as JSON into obscura-prover, rename output."""
    rc, stderr = gw._run_rust_prover(prover_dict, CIRCUIT_JSON, out_witness_gz)
    if rc != 0:
        print("[✗] obscura-prover failed:")
        print(stderr)
        sys.exit(1)
