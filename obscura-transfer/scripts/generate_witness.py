...
This is acceptable for local development only. For production:
  ...
  - Use HD key derivation (BIP-32 style) so users only need one seed phrase:
    sk_note = hash("Obscura::note_sk" || master_seed || note_index)
...
the entire privacy and security model of Obscura. All user funds would be
at immediate risk from malware, compromised backups, and device theft.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

How it works:
  1. Writes raw note data to obscura-hasher/Prover.toml (sk included)
  2. Runs nargo execute in obscura-hasher to compute hashes
  3. DELETES obscura-hasher/Prover.toml immediately
...
Requirements:
  ...
  - bb at ~/.bb/bb -- needed for proving (used directly or via obscura-prover)
  - obscura-prover (optional but recommended): cargo build --release in obscura-prover/

Proving strategies (auto-selected, best security first):
  1. Rust full-prove (default when obscura-prover binary exists):
     `obscura-prover prove` handles witness + prove + verify in one shot.
     Force with --rust-prove, disable with --witness-only.
  2. Rust witness-only (--witness-only):
     obscura-prover solves witness, Python calls bb separately.
...
HASHER_DIR   = os.path.join(os.path.dirname(TRANSFER_DIR), "obscura-hasher")
...
def run_hasher(inputs: dict) -> tuple:
    """
    Run obscura-hasher via `nargo execute` and return 15 public outputs:
    ...
    if not os.path.isdir(HASHER_DIR):
        print(f"[✗] obscura-hasher not found at:\n    {HASHER_DIR}")
        print("\n    Expected layout:")
        print("    ~/Desktop/obscura/")
        print("    ├── obscura-hasher/   <- must exist")
        print("    └── obscura-transfer/")
        sys.exit(1)
    ...
    if _fifo_available():
        ...
        print("[->] Running nargo execute in obscura-hasher (computing hashes)...")
        r = subprocess.run(
    ...
    if returncode != 0:
        ...
        print(combined)
        print("\nCommon fixes:")
        print("  - Circuit compile error -> check obscura-hasher/src/main.nr")
...
RUST_PROVER_BIN = os.path.join(
    ...
    "obscura-prover", "target", "release", "obscura-prover",
)


def _rust_prover_available() -> bool:
    """Check if the Rust obscura-prover binary is built and available."""
...
def _run_rust_prover(
    ...
    """
    Run the Rust obscura-prover to solve the witness via ACVM.
...
def _run_rust_prover_prove(
    ...
    """
    Run the Rust obscura-prover `prove` subcommand for the full pipeline.
...
def build_and_write(in1_value=100, in2_value=50,
    ...
    if kc:
        for label, buf in [
            ("obscura_in1_sk",  in1_sk_buf),
            ("obscura_in2_sk",  in2_sk_buf),
            ("obscura_out1_sk", out1_sk_buf),
            ("obscura_out2_sk", out2_sk_buf),
        ]:
...
def _bb_write_vk_prove_verify():
    ...
    run(
        [BB_BIN, "write_vk",
         "-b", "target/obscura_transfer.json",
         "-o", "target/vk"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Use Rust obscura-prover `prove` subcommand for the full pipeline "
             "(witness + prove + verify in one shot, no separate bb calls needed)"
    )
    parser.add_argument(
        "--witness-only", action="store_true",
        help="Use Rust obscura-prover for witness solving only (legacy behavior), "
    ...
    #   1. Rust full-prove: `obscura-prover prove` handles entire pipeline
    #      (witness + bb prove + verify + secure delete) in one shot.
    #      sk exists only in Rust process memory. No separate bb calls.
    #   2. Rust witness-only: `obscura-prover` solves witness via ACVM,
    ...
    if args.rust_prove and not use_rust_prover:
        print("[!] --rust-prove requested but obscura-prover not found at:")
        print(f"    {RUST_PROVER_BIN}")
        print("    Build with: cd obscura-prover && cargo build --release")
        sys.exit(1)
    if args.rust_prove:
        use_rust_prove = True
    ...
    if use_rust_prove and isinstance(result[0], dict):
        ...
        print()
        print("[RUST] Full pipeline via `obscura-prover prove`...")
        ...
        if returncode != 0:
            print(f"[x] obscura-prover prove failed (exit {returncode})")
            sys.exit(1)
    ...
    # Then Python calls bb CLI separately for proving/verifying.
        ...
        print()
        print("[RUST] Piping inputs to obscura-prover (witness-only mode)...")
        returncode, stderr = _run_rust_prover(
            prover_data, circuit_json, witness_gz
        )
        if returncode != 0:
            print(f"[x] obscura-prover failed:\n{stderr}")
            sys.exit(1)
